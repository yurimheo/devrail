...
const learningPath = [
  { name: 'Docker역', description: '컨테이너화 기술 학습', route: 'docker', isDisabled: false },
  { name: 'Kubernetes역', description: '컨테이너 오케스트레이션', route: 'kubernetes', isDisabled: false },
  { name: 'Jenkins역', description: '지속적 통합 및 배포', route: 'jenkins', isDisabled: false },
  { name: 'ArgoCD역', description: '지속적 배포 자동화', route: 'argocd', isDisabled: false },
  { name: 'Ansible역', description: '구성 관리 및 자동화', route: 'ansible', isDisabled: false },
            ...
            >
              ...
              >
                <h3 className={`text-xl font-bold ${station.isDisabled ? 'text-gray-400' : 'text-black'}`}>
                  {station.name}
                  {station.isDisabled && <span className="text-xs text-gray-500 ml-2">(현재 개발 중)</span>}
                </h3>
                ...
                <Button
                  ...
                  onClick={() => handleButtonClick(station.route, station.isDisabled)}
                >
                  탑승하기
                </Button>
              </div>
              ...
              {station.isDisabled && (
                <div className="absolute left-1/2 transform -translate-x-1/2 mt-4 w-4 h-4 bg-gray-400 rounded-full">
                  <img src="/images/under-construction.png" alt="Under Construction" className="w-full h-full" />
                </div>
              )}
            </div>
          ))}
