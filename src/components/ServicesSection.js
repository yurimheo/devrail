...
function CardTitle({ children, isDisabled }) {
  return (
    <h3
      className={`text-lg font-bold ${isDisabled ? 'text-gray-400' : 'text-black'}`}
    >
      {children}
    </h3>
  );
...
const services = [
  ...
  { name: 'Git', icon: '/images/services-icons/git-icon.svg', route: 'git' },
  {
    name: 'Terraform',
    icon: '/images/services-icons/terraform-icon.svg',
    route: 'terraform',
    isDisabled: true,
  },
  {
    name: 'Monitoring',
    icon: '/images/services-icons/monitoring-icon.svg',
    route: 'monitoring',
    isDisabled: true,
  },
  {
    name: 'Mysql',
    icon: '/images/services-icons/mysql-icon.svg',
    route: 'mysql',
    isDisabled: true,
  },
];
          ...
          {services.map((service) => (
            <Card key={service.name} isDisabled={service.isDisabled}>
              ...
              <CardContent>
                <CardDescription>
                  {service.isDisabled
                    ? `이 과목은 현재 준비 중입니다. 잠시만 기다려 주세요. ( •̀ ω •́ )✧`
