...
export default function WorkspaceShellPage() {
  const { workspace_id, courseId, dayNumber } = useParams();
  const navigate = useNavigate();

  // ✅ 현재 과목과 현재 DAY 찾기
  const course = courses.find((c) => c.id === courseId);
  const currentDay = course?.outline.find(
    (o) => o.day === parseInt(dayNumber, 10),
  );
  ...
  useEffect(() => {
    if (currentDay) {
      const markdownPath = `/markdowns/${courseId}/day${currentDay.day}.md`;
      fetch(markdownPath)
        .then((res) => res.text())
        .then((data) => {
        ...
        <div className="w-1/2 p-6 border-r bg-gray-50 overflow-y-auto">
          ...
          <div className="mt-6 flex justify-between">
            <button
              ...
              disabled={currentPage === 0 && parseInt(dayNumber, 10) === 1}
            >
              이전
            </button>
            <button
